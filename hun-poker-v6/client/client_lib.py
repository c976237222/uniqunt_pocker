    ...
    def handle_room_message(self, event: str, data: Dict, rid: str):
        match event:
            ...
            case SendEventEnum.UPDATE_TABLE_INFO:
                ...
                cards_info = self.player.get_joined_room_attr(rid, AttrEnum.cards)
                # 记录日志
                if type(data) != dict or type(cards_info['Cards']) != list:
                    print(data)
                else:
                    ...
                    data['CardInfo'] = cards_info

                    save_dir = f'./log/{rid}/{game_index}/'
                    filename = f'{time_stamp}.npy'
                ...
                if type(data) == dict:
                    num_round   = data['GameStatus']['Round']
                    last_action = data['GameStatus']['LastAction']

                    if 'Type' not in data['GameStatus']['LastAction'].keys():
                        print(f'[{time.asctime()}]','waiting for game start...')
                    elif data['GameStatus']['Round'] == 4:
                        pass
                    elif data['GameStatus']['Round'] == 101:
                        print(util.final_table_info_to_str(data))
                        print('\n')
                        pass
                    else:
                        self.ai.record_action_history(data)
            ...
            case SendEventEnum.UPDATE_ACTION:
                ...
                # print(data)
                print(util.final_info_to_str(data))
    ...
    def handle_user_message(self, event: str, data: Dict, rid: str):
        ...
        match event:
            case SendEventEnum.PLAY_ACTION:
                ...
                game_info = util.GameInfo(table_info, cards_info)
                if table_info['GameStatus']['Round'] not in [0, 1, 2, 3]:
                    return
            ...
            case SendEventEnum.UPDATE_USER_CARDS:
                ...
                cards_info=data['Cards'][0]
                self.ai.cards_info = cards_info
                self.write_log(
                    data, f"{SendEventEnum.UPDATE_USER_CARDS} on_user_message")
                # print('发牌信息：', data)       

                # {'Rid': 'ik6tcjI3', 'Cards': [{'uid': 'stbgb3kt', 'seatId': 3, 'card': [13, 23]}]}     
                # 记录发牌信息
                # game_index = data['GameIndex']
                # time_stamp = data['UpdateTimeStamp'] % 100000
...
class Player:
    ...
    def main_create_room(self,room_config, room_num):
        for i in range(room_num):
            self.create_room(room_config)
            print('room created', i+1)
        data=self.get_joined_room_list()
        self.config['join_rooms']=[{'rid':rid, 'pin':room_config['pin']} for rid in data['data']]
        with open(self.config_file,'w',encoding='utf-8') as f:
            yaml.dump(data=self.config,stream=f,allow_unicode=True)
    ...
    def join_target_room(self, room_idx):
        rooms: List[Dict] = self.config.get("join_rooms")
        if room_idx >= len(rooms):
            print('ERROR: room_idx exceed total number!!!')
            exit(0)
        room = rooms[room_idx]
        rid = room.get("rid")
        pin = room.get("pin")
...
def start_play(username, ai, room_idx):
    p = Player(username, ai)
    time.sleep(0.5)
    try:
        p.join_target_room(room_idx)
    except Exception as e:
        print(traceback.print_exc(), e)
