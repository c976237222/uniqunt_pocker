...
import os
from poker_ai.card import hand_to_rep
...
class Strategy:
    ...
    # def build_standard_range(self, range_dir = './range/GTO/CO'):
    #     range_dict = {}

    def update_range(self, cur_bet):
        bet_list = [100, 300, 1000, 10000, 100000, 1000000]
        clevel = 0
        for i in range(len(bet_list) - 1):
            if cur_bet < bet_list[i]:
                clevel = i
                break
        return self.preflop_range_dict[str(bet_list[clevel])]

    def build_preflop_range(self, range_dir = './range/BASE'):
        # 加载翻前范围表
        action_list = ['100', '300', '1000', '10000', '100000','1000000']

        preflop_range_dict = {}
        for action in action_list:
            preflop_range_dict[action] = Range(os.path.join(range_dir, action)+'.txt')
    ...
    def preflop(self, game_info:GameInfo, ar:ActionRecord, raise_num, **kwargs):
        ...
        effect_stack = game_info.effect_stack
        ...
        call_range = self.preflop_range_dict[str(bet_list[clevel])]
        # raise_range  = self.preflop_range_dict[str(nlevel)]
        ...
        if odds < 0.3 and bet_limit[0] < 100:
            call_range = self.preflop_range_dict['300']
            if my_cards in call_range.range_exp.keys():
                cf = call_range[my_cards]

        # 大手对的玩法
        rep = hand_to_rep(my_cards)
        if effect_stack < 3000: # 短码打法
            rf_dict = {'AA':1.0, 'KK':1.0, 'QQ':1.0, 'JJ':0.5}
        elif effect_stack < 10000: # 中等筹码,
            rf_dict = {'AA':1.0, 'KK':1.0, 'QQ':0.3}
        else: # 深筹
            rf_dict = {'AA':1.0, 'KK':0.3}
        if rep in rf_dict:
            rf = rf_dict[rep]
            cf = 1 - rf
            target = max(3 * pot_size, 0.25 * effect_stack)
            pr = target / pot_size
    ...
    def flop(self, game_info:GameInfo, ar:ActionRecord, win_rate, draw_rate, raise_num, **kwargs):
        ...
        print('odds={:.2f}, {:.2f}/{:.2f}'.format(odds, bet_limit[0], pot_size))
    ...
    def turn(self, game_info:GameInfo, ar:ActionRecord, win_rate, draw_rate, raise_num, **kwargs):
        rf, cf, pr = self.flop(game_info, ar, win_rate, draw_rate, raise_num, **kwargs)
    ...
    def river(self, game_info:GameInfo, ar:ActionRecord, win_rate, draw_rate, raise_num, **kwargs):
        ...
        if num_alive > 2: # 多人池
            if raise_num == 0 and num_not_acted > 1: # 前位,没人动
                ...
                elif win_rate > 0.8: # 胜率较高时,造池,争取打光
                    rf = 1
                    cf = 0
                    if spr > 16: # 翻牌圈打1/8后手就能在后两条街打光,这里调的大一点,争取转牌干光
                        pr = spr / 8
        ...
        if num_alive == 2: # 只剩两个人的情况
            if raise_num == 0 and num_not_acted > 1: # 前位
                ...
                if win_rate + draw_rate >= 0.97: # 手持nuts
                    ...
                    cf = 0
                    pr = max(spr/3, 5)
                elif win_rate > 0.85: # 胜率较高时,造池,争取打光
                    rf = ( win_rate + 0.1 ) ** raise_num
                    rf = min(1, rf)
                    cf = 1 - rf
                    if spr > 16: # 翻牌圈打1/8后手就能在后两条街打光
